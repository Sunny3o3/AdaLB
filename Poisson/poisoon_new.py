...
# =========================
# Imports adapted to your current project structure
# =========================
...
ADAM_STAGE_EPOCHS = 2500
ADAM_STAGE_LR = 1e-3

# ---- AdaLB -> L-BFGS settings ----
ADALB_STAGE_EPOCHS = 2500
ADALB_STAGE_LR = 1e-3
...
class ODE_Net(nn.Module):
    def __init__(self, hidden_units=20, num_hidden_layers=8):
        super().__init__()
        self.input_layer = nn.Linear(1, hidden_units)
        self.hidden_layers = nn.ModuleList(
            [nn.Linear(hidden_units, hidden_units) for _ in range(num_hidden_layers - 1)]
        )
...
def rhs_f(x: torch.Tensor) -> torch.Tensor:
    # For -u_xx = f(x), exact solution:
    # u(x) = sin(0.7x) + cos(1.5x) - 0.1x
    # => u_xx = -0.49 sin(0.7x) - 2.25 cos(1.5x)
    # => f(x) = 0.49 sin(0.7x) + 2.25 cos(1.5x)
    return 0.49 * torch.sin(0.7 * x) + 2.25 * torch.cos(1.5 * x)


def make_training_points(n_points=5000):
    return torch.linspace(X_LEFT, X_RIGHT, n_points, device=DEVICE).unsqueeze(1)
...
class PINNPoisson:
    def __init__(self, optimizer_name="adalb", gamma=0.99):
        ...
        self.x_interior = make_training_points(N_INTERIOR)
    ...
    def build_optimizer(self, name: str):
        ...
        if name == "adam":
            ...
            if AdaLB_xr is None:
                raise ImportError("AdaLB_xr is not available. Please provide AdaLB_xr.py or define AdaLB_xr in optimal.py.")
    ...
    def boundary_loss(self) -> torch.Tensor:
        x_left = torch.tensor([[X_LEFT]], device=self.device)
        x_right = torch.tensor([[X_RIGHT]], device=self.device)

        y_left = self.model(x_left)
        y_right = self.model(x_right)

        u_left = exact_solution(x_left)
        u_right = exact_solution(x_right)

        loss_bc = (y_left - u_left).pow(2) + (y_right - u_right).pow(2)
        return loss_bc.mean()

    def compute_loss(self):
        r_interior = self.residual(self.x_interior)
    ...
    def train(self, epochs=5000):
        ...
        if self.optimizer_name in ["adam", "amsgrad", "adamw", "adabelief", "nosadam", "adalb", "adalb_xr"]:
...
def save_single_run(run_dir, pinn: PINNPoisson, seed, train_time):
...
if __name__ == "__main__":
    ...
    for i, seed in enumerate(SEEDS):
        ...
        pinn = PINNPoisson(optimizer_name=OPTIMIZER_NAME, gamma=GAMMA)
