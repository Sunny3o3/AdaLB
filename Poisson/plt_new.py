...
# 输出文件名
MAIN_FIG_NAME = "poisson_main_mean_curve.png"
MAIN_ZOOM_FIG_NAME = "poisson_main_mean_curve_zoom.png"

GAMMA_FIG_NAME = "poisson_gamma_mean_curve.png"
GAMMA_ZOOM_FIG_NAME = "poisson_gamma_mean_curve_zoom.png"

HYBRID_FIG_NAME = "poisson_hybrid_mean_curve.png"
HYBRID_ZOOM_FIG_NAME = "poisson_hybrid_mean_curve_zoom.png"
...
def collect_run_files(result_dir: str, optimizer: str, gamma=None):
    ...
    for name in os.listdir(result_dir):
        ...
        if gamma_str is not None:
            ...
            # 对 gamma=None 的优化器，兼容类似 adam_gamma0.99_seed0 这种目录命名
            if name.startswith(f"{optimizer}_gamma") and "_seed" in name:
                loss_path = os.path.join(full_dir, "loss_list.txt")
                if os.path.isfile(loss_path):
                    matched_files.append(loss_path)
...
def load_curves(result_dir: str, optimizer: str, gamma=None):
    """
    读取同一设置下多个 seed 的 loss 曲线
    返回:
      curves: shape = [num_runs, num_epochs]
      files:  对应文件路径
    自动截断到最短长度，便于计算 mean/std
    """
...
def prepare_series(result_dir: str, config_list):
    """
    config_list: [(label, optimizer, gamma), ...]
    """
    series = []
...
def plot_mean_curves(
    ...
    for item in series_list:
        ...
        if show_std_band:
            lower = np.clip(y - s, a_min=1e-16, a_max=None)
            upper = y + s
...
if __name__ == "__main__":
    ...
    plot_mean_curves(
        ...
        title="Poisson equation: mean loss curves over 5 seeds",
        xlim=None,
        ylim=None,
    ...
    plot_mean_curves(
        ...
        title="Poisson equation: mean loss curves over 5 seeds (zoom)",
        xlim=(3000, 5000),
        ylim=(1e-4, 1e0) if USE_LOG_SCALE else (0, 2.0),
        use_log_scale=USE_LOG_SCALE,
        show_std_band=SHOW_STD_BAND,
    )
    ...
    plot_mean_curves(
        ...
        title="Poisson equation: sensitivity to $\\gamma$ (mean over 5 seeds)",
        xlim=None,
        ylim=None,
        use_log_scale=USE_LOG_SCALE,
        show_std_band=SHOW_STD_BAND,
    ...
    plot_mean_curves(
        ...
        title="Poisson equation: sensitivity to $\\gamma$ (zoom)",
        xlim=(3000, 5000),
        ylim=(1e-4, 1e0) if USE_LOG_SCALE else None,
        use_log_scale=USE_LOG_SCALE,
        show_std_band=SHOW_STD_BAND,
    )

    # -----------------------------
    # 3) Hybrid comparison figure
    # -----------------------------
    try:
        ...
        plot_mean_curves(
            ...
            title="Poisson equation: L-BFGS-based pipelines (mean over 5 seeds)",
            xlim=None,
            ylim=None,
            use_log_scale=USE_LOG_SCALE,
            show_std_band=SHOW_STD_BAND,
        ...
        plot_mean_curves(
            ...
            title="Poisson equation: L-BFGS-based pipelines (zoom)",
            xlim=(2000, 3000),
            ylim=(1e-6, 1e-1) if USE_LOG_SCALE else None,
            use_log_scale=USE_LOG_SCALE,
            show_std_band=SHOW_STD_BAND,
        )
